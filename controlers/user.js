  ...
  try {
    ...
    if (user) {
      ...
      const mailer = new Mailer(
        emailHeader,
        formatEmail(templateTypes.RESET_LINK, emailBody)
      );
...
const get_verify_reset_token = async (req, res) => {
  try {
    ...
    const mailer = new Mailer(
      newEmail,
      formatEmail(templateTypes.CONFIRM_ACTIVATION, emailBody)
    );
  ...
  try {
    ...
    if (!user) {
      return res.status(403).json({
        success: false,
        message:
          "Unable to update your password. Your session must have expired."
      });
    }
    ...
    const mailer = new Mailer(
      emailHeader,
      formatEmail(templateTypes.PASSWORD_CHANGED, emailBody)
    );
...
const get_activate_account = async (req, res) => {
  try {
    ...
    if (
      ...
      !user.tokens[0].expiration > Date.now()
    ) {
      ...
      const mailer = new Mailer(
        emailHeader,
        formatEmail(templateTypes.ACCOUNT_ACTIVATION_URL_EMAIL, emailBody)
      );
  ...
  try {
    ...
    req.user.comparePassword(password, async (err, isMatch) => {
      ...
      if (
        ...
        const mailer = new Mailer(
          emailHeader,
          formatEmail(templateTypes.ACCOUNT_UPDATED, emailBody)
        );
...
module.exports = app => {
  ...
  app.get(
    `/api/v1/users/account/user-profile`,
    authenticate,
    get_a_user_profile
  );
  app.post(
