...
validations.USER_SIGNUP = [
	body("name", "First name should be 2-30 characteres").isLength({ min: 2, max: 30 }).escape(),
...
validations.USER_RECOVERY = [ body("email", "The email address provided is not valid").isEmail() ];

validations.USER_RESET_NEW_PWD = [
	body("password", "Your new password is not valid").isLength({ min: 6 }),
	body("token", "Your session must be expired").isLength(5)
];
